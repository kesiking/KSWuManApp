//
//  ManWuTradeSKUSelectionControl.m
//  basicFoundation
//
...
@interface ManWuTradeSKUSelectionControl ()<TBSelectionControlDelegate>
...
@implementation ManWuTradeSKUSelectionControl
 ...
 * 创建属性值选择控件填充的值
 */
-(NSMutableArray *)createPropValueArray:(ManWuCommoditySKUModel *)skuProperty
{
    ...
    for (TBDetailSkuPropsValuesModel * value in skuProperty.values) {
        if (value.valueAlias.length > 0) {
            ...
            [valueNames addObject:value.valueId];
        }
    }
    return valueNames;
}
 ...
 *  创建属性选择控件
 */
-(TBTradeSKUPropSelectControl *)createPropSelectControl:(ManWuCommoditySKUModel *)skuProperty
{
...
- (void)createSkuPropertyValueControl{
    ...
    for (ManWuCommoditySKUModel *skuProperty in self.detailModel.skuArray) {
        /*创建某个属性选择的提示信息*/
...
- (void)clickedControl:(UIControl *)control index:(NSUInteger)index{
    ...
    ManWuCommoditySKUModel *skuPropsModel = self.detailModel.skuArray.count >  pidIndex ? [self.detailModel.skuArray objectAtIndex:pidIndex]:nil;
