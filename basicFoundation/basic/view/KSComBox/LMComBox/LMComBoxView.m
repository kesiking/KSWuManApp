...
#define safeStringForSting(x) [NSString stringWithFormat:@"%@",x]
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if(cell==nil)
    {
        ...
        UILabel *label = [[UILabel alloc]initWithFrame:CGRectMake(2, 0, self.frame.size.width-imgW - 5 - 2, self.frame.size.height)];
    ...
    label.text = safeStringForSting([_titlesList objectAtIndex:indexPath.row]);
...
{
    titleLabel.text = safeStringForSting([_titlesList objectAtIndex:indexPath.row]);
    _isOpen = YES;
    [self tapAction];
...
-(UIView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event{
    ...
    BOOL isTextViewInsideHitPoint = CGRectContainsPoint(self.listTable.frame, hitPoint);
