...
-(void)setMenuContainerViewPoint:(CGPoint)menuTargetPoint{
    CGFloat startX = menuTargetPoint.x > 0 ? menuTargetPoint.x : CGRectGetWidth(self.bounds) - kXHMenuTableViewWidth - 6;
    CGFloat startY = menuTargetPoint.y > 0 ? menuTargetPoint.y : 65;
    self.menuContainerView.frame = CGRectMake(startX, startY, kXHMenuTableViewWidth, self.menus.count * (kXHMenuItemViewHeight + kXHSeparatorLineImageViewHeight) + kXHMenuTableViewSapcing);
}
